...
const productclicked = () => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        div.innerHTML =
            ...
            "<div id = 'producttext'><div class='titles'>" + data[i].Title +  "<button id = 'buy_button' onclick='buy()'>Buy</button></div>" + "<br/>" +
...
const register = () => {
    ...
    let reg = {"Address":"String content", "Name": uname , "Password": pwd}

    const regPromise = fetch('http://redsox.uoa.auckland.ac.nz/ds/DairyService.svc/register',
    {
        headers: {
            // "Accept": 'application/json',
            "Content-Type": "application/json",
...
const sign_in_clicked = () => {
    const uname = document.getElementById('uname').value;
    const pwd = document.getElementById('pwd').value;
    const sign_in_Fetch = fetch('http://redsox.uoa.auckland.ac.nz/dsa/Service.svc/user',
        {
            headers: { "Accept": "application/json" },
        });
    const sign_in_Promise = sign_in_Fetch.then((response) => response.text());
    sign_in_Promise.then((data) => {
        console.log(data)
    });
}

// http://redsox.uoa.auckland.ac.nz/dsa/Service.svc/user
